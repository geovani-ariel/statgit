test_that("find_rstudio_projects lista arquivos .Rproj encontrados", {
  ...
  projects <- find_rstudio_projects(root)

  expect_equal(nrow(projects), 2)
  expect_equal(projects$name, c("a", "b"))
})

test_that("open_stats_project retorna o caminho quando RStudio nao esta disponivel", {
  root <- withr::local_tempdir()
  ...
  result <- open_stats_project(project_file)
...
test_that("open_stats_project usa openProject quando RStudio esta disponivel", {
  root <- withr::local_tempdir()
  project_file <- file.path(root, "analise.Rproj")
  writeLines("Version: 1.0", project_file)
  opened <- NULL
