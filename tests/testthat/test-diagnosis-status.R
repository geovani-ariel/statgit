test_that("git_check informa ausência de repositório Git", {
...
test_that("git_check detecta remote e repositorio acima da subpasta", {
  with_isolated_git_identity({
    project <- withr::local_tempdir()
...
test_that("git_status descreve arquivos novos e modificados", {
  with_isolated_git_identity({
    ...
    output <- capture.output(git_status(project))
...
test_that("git_status descreve staged e removidos", {
  with_isolated_git_identity({
    project <- withr::local_tempdir()
    git_init(project, branch = "main")
    ...
    git_stage("README.md", path = project)
